...
from ecrterm.exceptions import TransportConnectionFailed, TransportLayerException
...
def ecr_log(data, incoming=False, **kwargs):
    try:
        ...
        try:
            ...
            _logger.debug("Cannot be represented: %s", data)
            _logger.error(e)
...
class ECR:
            ...
            for inc, packet in self.transmitter.last_history:
                if inc and isinstance(packet, Completion):
                    if "tid" in packet.bitmaps_as_dict().keys():
                        self.terminal_id = (
                            packet.bitmaps_as_dict().get("tid", BCD(0)).value()
                        )
            # remember this.
    ...
    def register_unlocked(self):
        ...
        ret = self.transmit(
            Registration(
                ...
                config_byte=Registration.generate_config(ecr_controls_admin=False),
            )
        )
        if ret == TRANSMIT_OK:
            self._state_registered = True
    ...
    def end_of_day(self, listener=None):
        ...
        if not self.daylog:
            ...
            try:
                self.daylog = [
                    x for x in (self.daylog_template % eod_info).split("\n") if x
                ]
            except Exception as e:
                _logger.exception(e)
        return result
    ...
    def last_printout_with_attribute(self):
        ...
        for entry in self.transmitter.last_history:
            ...
            if inc and isinstance(packet, PrintLine):
                printout += [
                    (packet.fixed_values["attribute"], packet.fixed_values["text"])
                ]
        return printout
    ...
    def last_intermediate_status_information(self):
        ...
        for entry in self.transmitter.last_history:
            ...
            if inc and isinstance(packet, IntermediateStatusInformation):
                packet_dict = {b._key: b.value() for b in packet.bitmaps}
                status_information += [
                    packet.fixed_values.get("intermediate_status", None)
                ]
        return status_information
